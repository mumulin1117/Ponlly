...
struct PonllyContentReportContext {
    let title: String
    let subtitle: String?
...
final class PonllyReportRoomViewController: UIViewController, UITextViewDelegate {
    private let context: PonllyContentReportContext
    ...
    init(room: PonllyVoiceRoom) {
        ...
        self.context = PonllyContentReportContext(
            title: room.title,
    ...
    private func setup() {
        ...
        ["Inappropriate Content", "Harassment", "Advertising / Spam", "Other"].forEach { reason in
            let row = PonllyReportReasonRow(reason: reason)
    ...
    @objc private func reasonTapped(_ sender: PonllyReportReasonRow) {
