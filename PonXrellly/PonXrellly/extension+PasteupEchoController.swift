...
extension PasteupEchoController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: MMuralGridell.reuseId, for: indexPath) as! MMuralGridell
        ...
        cell.onReport = { [weak self] in
            guard let self else { return }
            self.requireVideoLogin {
                let report = PonllyroughFillController(video: video)
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        (cell as? MMuralGridell)?.play()
    }

    func collectionView(_ collectionView: UICollectionView, didEndDisplaying cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        (cell as? MMuralGridell)?.pause()
    }
...
private extension PasteupEchoController {
    func presentComments(for video: PonllyVideo) {
        requireVideoLogin {
            let sheet = PonllyletterFormController(video: video) { [weak self] updated in
                self?.updateVideo(updated)
            }
